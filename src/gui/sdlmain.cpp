...
int main(int argc, char* argv[]) SDL_MAIN_NOEXCEPT {
...
#if defined(MACOSX) || defined(LINUX) || (defined(WIN32) && !defined(HX_DOS))
    {
        if(control->opt_promptfolder < 0) {
            ...
            std::string res_path;
            try {
                res_path = Cross::GetPlatformResDir2();
            }
            catch(...) {
                //LOG(LOG_GUI, LOG_WARNING)("Cross::GetPlatformResDir2 failed, skipping config check.");
                res_path.clear();
            }
            ...
            if(!res_path.empty()) {
                struct stat st;
                try {
                    std::string cfgfile = res_path + "dosbox-x.conf";
                    if(stat(cfgfile.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
                        control->opt_promptfolder = 0;
                    }
                }
                catch(...) {
                    //LOG(LOG_GUI, LOG_WARNING)("Invalid path, skipping dosbox-x.conf check.");
                }
            }
            else {
                //LOG(LOG_GUI, LOG_WARNING)("res_path is empty, skipping dosbox-x.conf check.");
            }
        }
